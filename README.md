...
d'usage**, et **panel d'admin web LAN-only**. Le TLS public (`llm.lelabs.tech`) est terminé par
...
Voir [docs/DAT.md](docs/DAT.md) pour le détail (services, données, lancement, déploiement Jetson).
...
- `./runProd` — sur la Jetson : Caddy (DNS-01 Scaleway) + proxy + admin en `network_mode: host`.
