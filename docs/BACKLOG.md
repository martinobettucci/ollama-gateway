...
- [~] **Build des images sur la Jetson (aarch64)** — à valider `docker compose build` sur la box
  (Docker Desktop indisponible côté dev WSL ; app pur Python → build attendu OK). Reste à exécuter.
- [~] **Déploiement prod Jetson** : import clé Gram, retrait nginx, `runProd`, cert DNS-01, preuves
